                ...
                try {
                    if(wkt.read(geometria_ini).isValid()){
                        ...
                        try {
                            ...
                            String identificador=main.polygon.get(main.polygon.size()-1).getId();
                            String descripcion=c.getString(c.getColumnIndex(Estructura.NovedadEntry.DESCRIPCION));

                                if(c.getString(c.getColumnIndex(Estructura.NovedadEntry.TIPO)).equals("ENA_F")){
                                    String centroide=main.analisis.centroidePoligono(main.polygon.get(main.polygon.size()-1));
                                    int color_ena_cerrado =  main.getResources().getColor(R.color.poligono_cerrado_ena);
                                    main.polygon.get(main.polygon.size()-1).setFillColor(color_ena_cerrado);
